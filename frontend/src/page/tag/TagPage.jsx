...
import { BarChart3, CheckCircle2, Copy, ExternalLink, Pencil, Plus, Radio, RotateCcw, Tags, Trash2 } from 'lucide-react'
...
const emptyForm = { nickname: '', type: TAG_SERIES, useTag: true }
...
function TagPage() {
  ...
  const loadTags = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const response = await getTags(storeId, 'ALL')
  ...
  const openForm = (tag = null) => {
    setEditing(tag)
    setForm(tag
      ? { nickname: tag.nickname, type: tag.category || TAG_SERIES, useTag: tag.used ?? tag.isUsed }
      : { ...emptyForm })
    setModalOpen(true)
  }
  ...
  return (
    <div className="page">
      <div className="page-heading">
        <div><span className="eyebrow">MANAGEMENT</span><h1>태그관리</h1><p>매장별 태그카드와 사용 상태를 관리합니다.</p></div>
      ...
      <section className="panel">
        ...
        <div className="table-wrap">
          <table>
            <thead><tr><th className="check-cell"><input type="checkbox" checked={allSelected} onChange={(e) => setSelected(e.target.checked ? tags.map((tag) => tag.id) : [])} /></th><th>태그 별칭</th><th>시리즈</th><th>카드 타입</th><th>태그 URL</th><th>접속 수</th><th>상태</th><th /></tr></thead>
            <tbody>
              {loading ? <tr><td colSpan="8" className="empty">태그 정보를 불러오는 중입니다.</td></tr>
                : !storeId ? <tr><td colSpan="8" className="empty">태그를 조회할 매장을 선택하세요.</td></tr>
                  : tags.length === 0 ? <tr><td colSpan="8" className="empty">이 매장에 등록된 태그카드가 없습니다.</td></tr>
                    : tags.map((tag) => {
      ...
      {modalOpen && (
        <Modal title={editing ? '태그 정보 수정' : '새 태그 등록'} description={editing ? '태그 별칭과 사용 상태를 변경합니다.' : '선택한 매장에 새 태그를 연결합니다.'} onClose={() => setModalOpen(false)}
          ...
          <form id="tag-form" className="form-grid" onSubmit={submit}>
            ...
            <label>시리즈<input value={form.type || TAG_SERIES} disabled readOnly /></label>
      ...
      {createdTagUrl && (
        ...
        >
          <div className="tag-create-result">
            <img src={nfcRegistrationGuide} alt="스마트폰으로 태그카드를 등록하는 방법" />
            <div className="tag-create-guide">
              <CheckCircle2 size={22} />
              <div>
                <strong>태그에 URL을 등록해 주세요</strong>
                <p>모바일 NFC Tools 등으로 아래 URL을 태그카드에 등록해 주세요.</p>
              </div>
            </div>
