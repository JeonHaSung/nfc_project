...
public interface RedirectingRepository extends JpaRepository<RedirectingEntity, Long> {
    ...
    Optional<RedirectingEntity> findFirstByTagIdAndQuickTrueAndDelFalse(String tagId);
    ...
    boolean existsByRedirectingTypeId(Long redirectingTypeId);

    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("UPDATE RedirectingEntity r SET r.redirectingTypeId = :typeId "
            + "WHERE r.redirectingTypeId IS NULL AND r.redirectingType = :code")
    int backfillTypeId(@Param("typeId") Long typeId, @Param("code") String code);
